...
// Grok API へのプロキシ。APIキーはVercel環境変数で管理する。

export default async function handler(req, res) {
  // CORS ヘッダー（GitHub Pages や localhost からも呼べるよう設定）
  ...
  if (!apiKey) {
    return res.status(500).json({ error: 'GROK_API_KEY が環境変数に設定されていません' });
  }
  ...
  try {
    ...
    if (!userMessage) {
      return res.status(400).json({ error: 'userMessage は必須です' });
    }
